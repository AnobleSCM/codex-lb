...
- [x] 1.2 Apply the resolver when issuing password and TOTP-verified dashboard sessions.
...
- [x] 1.5 Constrain the explicit loopback-host-header override so it never bypasses the socket-level loopback requirement.
...
- [x] 2.3 Update frontend test defaults to match the backend default.

## 3. Verification

- [x] 3.1 Add unit tests for direct loopback, bridge fallback, bridge fallback with override, spoofed remote Host, non-STANDARD auth mode, proxy, and shorter-TTL resolver behavior.
