...
The `request_decompression` middleware MUST enforce a hard ceiling on decompressed request body size to bound worst-case in-memory allocation from compressed payloads. The default ceiling MUST be high enough that the proxy-side `response.create` slimmer (`_slim_response_create_payload_for_upstream`) can run on realistic tool-heavy Codex CLI sessions — specifically, MUST be at least one order of magnitude above `Settings.upstream_response_create_max_bytes` so the slimmer is the effective enforcer of the upstream-correct payload size on `/backend-api/codex/responses` and `/v1/responses`. The default MUST NOT be set so low that a request which the slimmer could otherwise reduce below `upstream_response_create_max_bytes` is rejected by the middleware before the slimmer runs.
