...
def test_state_from_account_recovers_rate_limited_on_restart_without_blocked_at_when_usage_shows_new_reset_window(
    monkeypatch,
):
    now = 1_700_000_000.0
    future_reset = int(now + 3600)
...
def test_state_from_account_keeps_rate_limited_on_restart_when_fresh_primary_usage_is_missing_and_no_blocked_at(
    monkeypatch,
):
    now = 1_700_000_000.0
    future_reset = int(now + 3600)
...
def test_state_from_account_keeps_rate_limited_without_blocked_at_when_usage_stays_on_same_reset_window(
    monkeypatch,
):
    now = 1_700_000_000.0
    future_reset = int(now + 3600)
...
def test_state_from_account_keeps_rate_limited_after_restart_when_persisted_blocked_at_is_recent(monkeypatch):
    now = 1_700_000_000.0
    blocked = now - 60.0
    future_reset = int(now + 3600)
    monkeypatch.setattr("app.modules.proxy.load_balancer.time.time", lambda: now)
...
def test_state_from_account_keeps_rate_limited_after_restart_when_primary_usage_is_older_than_block(monkeypatch):
    now = 1_700_000_000.0
    blocked = now - 130.0
    future_reset = int(now + 3600)
    monkeypatch.setattr("app.modules.proxy.load_balancer.time.time", lambda: now)
...
def test_state_from_account_keeps_rate_limited_during_active_runtime_cooldown_even_when_db_threshold_elapsed(
    monkeypatch,
):
    now = 1_700_000_000.0
    blocked = now - 130.0
    ...
    # Runtime cooldown is in the future — a Retry-After-derived hold the
    # balancer should respect. The DB-derived RATE_LIMITED_COOLDOWN_SECONDS
    # threshold has elapsed (blocked=now-130s, threshold=120s), but the
    # in-memory cooldown MUST take precedence so the retry-after hold is
    # not shortened.
    runtime = RuntimeState()
    runtime.cooldown_until = now + 60.0
    runtime.blocked_at = blocked
...
def test_state_from_account_keeps_rate_limited_after_restart_when_primary_still_saturated(monkeypatch):
    now = 1_700_000_000.0
    blocked = now - 130.0
    future_reset = int(now + 3600)
    monkeypatch.setattr("app.modules.proxy.load_balancer.time.time", lambda: now)
    ...
    saturated_primary = _make_test_usage(
        window="primary",
        used_percent=100.0,
        reset_at=future_reset,
        recorded_at=_epoch_to_naive_utc(now - 30),
    ...
    state = _state_from_account(
        account=account,
        primary_entry=saturated_primary,
        secondary_entry=None,
        runtime=RuntimeState(),
    )
    assert state.status == AccountStatus.RATE_LIMITED
