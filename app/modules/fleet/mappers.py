...
def fleet_account_summary_from_account(account: AccountSummary) -> FleetAccountSummary:
    """Project a dashboard ``AccountSummary`` into the minimal fleet summary.

    Only the headroom/reset/status fields are carried over; token, auth,
    credit-balance, request-cost, and additional-quota fields on the source
